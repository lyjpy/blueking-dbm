...
class MongoDBListRetrieveResource(query.ListRetrieveResource, MongoDBExportQueryResourceMixin):
    ...
    def _filter_instance_qs(cls, query_filters: Q, query_params: Dict[str, str]) -> QuerySet:
        ...
        if mongodb_state:
            ...
            storage_qs = storage_instance.filter(*mongo_state_filters).distinct()
            if "__empty__" in values:
                return storage_qs.union(proxy_instance).order_by(ordering)

            return storage_qs.order_by(ordering)
